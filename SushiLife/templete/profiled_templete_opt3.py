...
from SushiLife import Updater, Agent, Exchange, DataAsset, load_data # Keep specific imports
from SushiLife.Account_opt3 import StockAccount # New StockAccount import

# Adjusted HDF5 path assuming execution from repo root
f = h5py.File("SushiLife/data/stock_info.hdf5", "r")
array_stock, axis_stock = load_data(f, "stock", chunks=5) # Keep original load_data chunks for now, or change if intended. This was not specified.
array_value, axis_value = load_data(f, "value", chunks=5) # Keep original load_data chunks for now, or change if intended. This was not specified.

data_stock = DataAsset(array_stock, axis_stock, chunks=60, in_memory=False) # Increased chunks
data_value = DataAsset(array_value, axis_value, chunks=60, in_memory=False) # Increased chunks
...
profiler_init.dump_stats("init_profile_opt3.prof")
profiler_loop.dump_stats("loop_profile_opt3.prof")

print("--- Initialization Profile Stats (opt3) ---")
s_init = io.StringIO()
ps_init = pstats.Stats(profiler_init, stream=s_init).sort_stats('cumulative')
ps_init.print_stats(15)
print(s_init.getvalue())

print("\n--- Main Loop Profile Stats (opt3) ---")
s_loop = io.StringIO()
